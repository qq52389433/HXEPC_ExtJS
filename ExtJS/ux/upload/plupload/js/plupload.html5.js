        ...
        init: function (p, r) {
            ...
            function o(w) {
                ...
                for (t = 0; t < w.length; t++) {
                    ...
                    ////判断是否是目录
                    //if (u.type==="" && u.size<32768)
                    //{
                    //    //判断浏览器的类型，是否使用cefSharp浏览
                    //    if (_browserName == "cefSharp") {
                    //        jsObj.onDocGridDropFolder(localStorage.getItem("sid"));
                    //    }
                    //    continue
                    //}
                    s[u.name] = true;
                    ...
                    c[x] = u;
                    //如果文件大小大于零，就添加文件
                    if ((u.fileSize || u.size) > 0)
                    {
                        v.push(new j.File(x, u.fileName || u.name, u.fileSize || u.size, u.lastModifiedDate, u.type))
                    }
                }
                if (v.length) {
            ...
            p.bind("Init", function (w) {
                ...
                //G.innerHTML = G.innerHTML + '<input type="hidden" id="theFilePath" name="theFilePath" value=""> ';
                G.scrollTop = 100;
                //p:文件选择窗口
                ...
                function w(A) {
                    ...
                    function B() {
                        ...
                        function G(V) {
                            ...
                            if (q.upload) {
                                q.upload.onprogress = function (W) {
                                    //W.loaded:XMLHttp已上传的大小，lastloaded：上传中断时上传的大小
                                    u.loaded = Math.min(u.size, C + u.lastloaded + W.loaded - T);
                            ...
                            q.onreadystatechange = function () {
                                var W, Y;
                                //q:XMLHttpRequest,readyState:
                                //0 － （未初始化）还没有调用send()方法
                                //1 － （载入）已调用send()方法，正在发送请求
                                //2 － （载入完成）send()方法执行完成，已经接收到全部响应内容
                                //3 － （交互）正在解析响应内容
                                //4 － （完成）响应内容解析完成，可以在客户端调用了
                                //,s:g.Uploader,上传时state为2,j.STOPPED:1
                                if (q.readyState == 4 && s.state !== j.STOPPED) {
                                    ...
                                    if (W >= 400) {
                                        ...
                                        if (K) {
                                            ...
                                            //如果还有文件块没有上传，继续上传文件块
                                            if (!s.trigger("ChunkUploaded", u, Y)) {
                                                //如果上传文件块错误，重新断点续传文件
                                                //u.status = j.FAILED;
                                                s.stop();
                                                s.start();
                            ...
                            if (s.settings.multipart && n.multipart) {
                                ...
                                if (typeof (V) === "string") {
                                    ...
                                    R += P + N + U + 'Content-Disposition: form-data; enctype="multipart/form-data"; name="' + s.settings.file_data_name + '"; filename="' + unescape(encodeURIComponent(u.name)) + '"' + U + "Content-Type: " + F + U + U + V + U + P + N + P + U;
                                    T = R.length - V.length;
                                    V = R;
                                    if (q.sendAsBinary) {
                                        q.sendAsBinary(V)
                        ...
                        if (v.chunk_size && u.size > v.chunk_size && (n.chunks || typeof (A) == "string")) {
                            //根据文件已上传的大小，判断从哪里开始剪切分块
                            D = (u.loaded / v.chunk_size);
                            H = v.chunk_size;
