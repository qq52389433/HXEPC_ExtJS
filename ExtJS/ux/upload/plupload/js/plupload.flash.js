            ...
            m.bind("Flash:Init", function () {
                ...
                m.bind("Flash:UploadProcess", function (t, s) {
                    ...
                    if (u.status != d.FAILED) {
                        //s.loaded:已上传的大小，lastloaded：上传中断时上传的大小
                        u.loaded = u.lastloaded + s.loaded;
                        u.size = s.size;
                ...
                m.bind("Flash:UploadChunkComplete", function (s, u) {
                    ...
                    //如果还有文件块没有上传，继续上传文件块
                    if (!s.trigger("ChunkUploaded", t, v)){
                        //如果上传文件块错误，重新断点续传文件
                        s.stop();
                        s.start();
                        return
                ...
                //按下flash按钮时触发这个事件,d:plupload主对象，s:g.uploader,
                ...
                m.bind("flash_click", function (s, t) {
                    //这里需要修改swf文件，调用flash的click事件，swf里面需要添加：
                    //flash.external.ExternalInterface.addCallback("flash_btn_click", this.flash_btn_click);
                    // n().flash_btn_click(t);
                });
