...
(function () {
                    ...
                    if (!w && t[v].status == g.QUEUED) {
                        ...
                        if (this.trigger("BeforeUpload", w)) {
                            ...
                            var int = window.setInterval(function () {
                                        if (w.status === g.UPLOADING) {
                                            me.trigger("UploadFile", w);
        ...
        g.extend(this, {
            ...
            trigger: function (w) {
                ...
                if (y) {
                    ...
                    //v[0]:事件名称，v[1]:文件g.File
                    v[0] = this;
                    //遍历运行该事件对象数组里面的对象，当数组里面有两个对象时，一个为客户定义事件对象，一个为系统默认事件对象
                    for (x = 0; x < y.length; x++) {
                        if (w === "BeforeUpload") {
                            if (y[x].func.apply(y[x].scope, v) === false) {
                                return false
                            }
                            //y[x].func.call(y[x].scope, v);
                        } else {
                            if (y[x].func.apply(y[x].scope, v) === false) {
                                return false
                            }
    ...
    //定义文件对象
    g.File = function (q, o, p, lastModified) {
        var n = this;
        n.id = q;
        n.name = o;
        n.size = p;
        ...
        n.percent = 0;
        n.createtime = "";
        n.updatetime = lastModified;
        n.type = "";
        //n.updatetime = new Date(parseInt(lastModified) ).toLocaleString().replace(/年|月/g, "-").replace(/日/g, " ");
        n.status = 0
    };
    g.Runtime = function () {
