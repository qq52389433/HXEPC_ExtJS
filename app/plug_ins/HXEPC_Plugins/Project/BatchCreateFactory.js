/*批量创建文档*/
Ext.define('Ext.plug_ins.HXEPC_Plugins.Project.BatchCreateFactory', {
    extend: 'Ext.container.Container',
    alias: 'widget.BatchCreateFactory',
    //layout: "border",
    layout: 'fit',
    ...
    initComponent: function () {
                            ...
                            if (nodes !== null && nodes.length > 0) {
                                ...
                                for (var i = 0; i < arr.length ; i++) {
                                    ...
                                    if (arrItem[0] != "") {
                                        ...
                                        rec.set('factoryEngDesc', arrItem[0]);
                                        rec.set('factoryCode', arrItem[1]);
                                        rec.set('factoryDesc', arrItem[2]);
        ...
        Ext.define("contentModel", {
            ...
            fields: ["id", "factoryCode", "factoryDesc", "factoryEngDesc", "tempDef"],
            url: "_blank",
        });
        ...
        me.contentGrid = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                {
                    header: '英文名', dataIndex: 'factoryEngDesc',  width: 321,
                    editor: me.CodeEditing
                },
                {
                    header: '代 码', dataIndex: 'factoryCode', width: 120,
                    editor: me.EnDescEditing
                },
                {
                    header: '描 述', dataIndex: 'factoryDesc',  width: 321,
    ...
    send_create_Doc: function () {
        ...
        if (batSend === true) {
            ...
            me.createDocIndex = 0;
            me.loop_create_factory(0);//循环创建文档
    ...
    //循环创建文档
    loop_create_factory: function (docIndex) {
        var me = this;
        var records = me.contentStore.getRange();
        if (docIndex >= records.length) {
            winBatchCreateFactory.close();
        } else {
            if (records[docIndex].data.factoryCode === "") {
                docIndex = docIndex + 1;
                //循环创建文档
                me.loop_create_factory(docIndex);
            } else {
                factoryAttr =
                [{ name: 'factoryCode', value: records[docIndex].data.factoryCode },
                    { name: 'factoryDesc', value: records[docIndex].data.factoryDesc },
                    { name: 'factoryEngDesc', value: records[docIndex].data.factoryEngDesc },
                ...
                Ext.Ajax.request({
                    ...
                    method: "POST",
                    params: {
                        C: "AVEVA.CDMS.HXEPC_Plugins.HXProject", A: "CreateFactory",
                        sid: localStorage.getItem("sid"), projectKeyword: me.projectKeyword, factoryAttrJson: factoryAttrJson,
                    },
    ...
    batch_send_create_Doc: function () {
        ...
        Ext.Ajax.request({
            ...
            method: "POST",
            params: {
                C: "AVEVA.CDMS.HXEPC_Plugins.HXProject", A: "BatchCreateFactory",
