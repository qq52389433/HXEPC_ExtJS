...
function selectProfession(mainPanelId, res) {
    ...
    if (plugins === "HXPC_Plugins" && DefWorkFlow === "CREATEPROJECT" && CuWorkState === "DESIGNDIRECTOR" && FuncName === "selectProfession") {
        ...
        var fmSelectProfession = Ext.create('Ext.plug_ins.m.' + plugins + '.' + FuncName, {
            title: "", mainPanelId: mainPanelId, projectDesc: projectDesc, projectKeyword: projectKeyword
        });
...
function selectUserEx(mainPanelId, res) {
    ...
    if (plugins === "HXPC_Plugins" && (DefWorkFlow === "EXCHANGEDOC" || DefWorkFlow == "SENDDOC" || DefWorkFlow == "WORKTASK") && CuWorkState === "DIRECTORSELECT" && FuncName === "selectUserEx") {
        //Ext.Msg.alert("返回信息", plugins + "," + DefWorkFlow + "," + CuWorkState + "," + state);
        var fmSelectUserEx = Ext.create('Ext.plug_ins.m.' + plugins + '.' + FuncName, { title: "", mainPanelId: mainPanelId, WfKeyword: WfKeyword });
...
function selectUserApprov(mainPanelId, res) {
    ...
    if (plugins === "HXPC_Plugins" && (DefWorkFlow === "PRODUCT" || DefWorkFlow == "PUBLICPRODUCT" ) && CuWorkState === "DIRECTORSELECT" && FuncName === "selectUserApprov") {
        //Ext.Msg.alert("返回信息", plugins + "," + DefWorkFlow + "," + CuWorkState + "," + state);
        var fmSelectUserApprov = Ext.create('Ext.plug_ins.m.' + plugins + '.' + FuncName, { title: "", mainPanelId: mainPanelId, WfKeyword: WfKeyword });
...
//弹出消息框，选择收文流程的参与专业
//调用本函数的位置在：Ext.ux.m.WorkFlow._WorkFlowPage.loadWorkflowAuditPage，里面的GotoNextWorkflowState的返回处理函数
function receiveDocSelectProfession(mainPanelId, res) {
   ...
   // Ext.Msg.alert("错误信息", "1");
    ...
    if (plugins === "HXPC_Plugins" && DefWorkFlow === "RECEIVEDOC" && CuWorkState === "PROJECTOWNER" && FuncName === "receiveDocSelectProfession") {
        ...
        //Ext.Msg.alert("返回信息", plugins + "," + DefWorkFlow + "," + CuWorkState + "," + state);
        //Ext.Msg.alert("错误信息", "1:" + plugins + "," + FuncName + ",\r\n " + mainPanelId + "," + wfKeyword);
        //Ext.Msg.alert("错误信息", "1:" + mainPanelId + "," + wfKeyword);

        FuncName = "recDocSelProfession";

        var fmRecDocSelProfession = Ext.create('Ext.plug_ins.m.' + plugins + '.' + FuncName, { title: "", mainPanelId: mainPanelId, wfKeyword: wfKeyword });
        //var fmSelProf = Ext.create('Ext.plug_ins.m.' + plugins + '.' + FuncName, { title: "", mainPanelId: mainPanelId, wfKeyword: wfKeyword });
        //var fmSelProf = Ext.create('Ext.plug_ins.m.HXPC_Plugins.recDocSelProf', { title: "" });
        ...
        winReceiveDocSelectProfession = Ext.widget('window', {
            ...
            modal: true,
            closeAction: 'close', //close 关闭  hide  隐藏  
            items: fmRecDocSelProfession,
            defaultFocus: 'firstName'
        });

        fmRecDocSelProfession.send_get_profession_list(function () {
...
function productSelectProfession(mainPanelId, res) {
    ...
    if (plugins === "HXPC_Plugins" && DefWorkFlow === "PRODUCT" && CuWorkState === "START" && FuncName === "productSelectProfession") {
        //Ext.Msg.alert("返回信息", plugins + "," + DefWorkFlow + "," + CuWorkState + "," + state);
        var fmProductSelectProfession = Ext.create('Ext.plug_ins.m.' + plugins + '.' + FuncName, { title: "", mainPanelId: mainPanelId, docKeyword: docKeyword });
...
//处理收文目录拖拽文件到DocGrid控件的事件（收文）
//调用本函数的位置在：Ext.ux.m.Doc._MainDocGrid 里面的 AfterCreateNewDocEvent 函数
...
function getDocument(mainPanelId, res,  files) {
    ...
    if (plugins === "HXPC_Plugins" && FuncName === "getDocument") {
        //Ext.Msg.alert("返回信息", plugins + "," + DefWorkFlow + "," + CuWorkState + "," + state);
        var fmGetDocument = Ext.create('Ext.plug_ins.m.' + plugins + '.' + FuncName, { title: "", mainPanelId: mainPanelId, docKeyword: DocKeyword });
...
//这一个定义必须保留在文件末尾
Ext.define('Ext.plug_ins.m.HXPC_Plugins.common', {
});
